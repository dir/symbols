...
const vscodeSourceThemePath = path.join(
  rootDir,
  "vscode-symbols/src/symbol-icon-theme.json",
);
...
fs.writeFileSync(
  vscodeModifiedThemePath,
  `${JSON.stringify(mergedTheme, null, "\t")}\n`,
);

console.log(
  `Applied custom mappings to ${path.relative(rootDir, vscodeModifiedThemePath)}`,
);
